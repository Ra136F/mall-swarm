node {
    ...
    stage('Pull') { // for display purposes
        checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [[$class: 'GitLFSPull',timeout:30]], userRemoteConfigs: [[credentialsId: 'e2a55341-7ddc-412c-90a9-07d8e0901a88', url: 'git@47.122.4.10:root/mall-swarm.git']]])
    }
    stage('Build') {
        sh "docker images"
//         sh "mvn -v"
